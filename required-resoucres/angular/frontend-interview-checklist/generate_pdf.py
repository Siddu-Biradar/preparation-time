...
HTML_PATH = BASE_DIR / "frontend-interview-checklist-guide.html"
PDF_PATH = BASE_DIR / "frontend-interview-checklist-guide.pdf"
...
def process_element(el: Tag, styles: StyleSheet1, story: list) -> None:
    """Process a single HTML element into ReportLab flowables."""
    name = el.name.lower()

    if name == "h1":
        story.append(Paragraph(paragraph_markup(el), styles["GuideTitle"]))
    elif name == "h2":
        if "section-break" in (el.get("class") or []):
            story.append(PageBreak())
        story.append(Paragraph(paragraph_markup(el), styles["GuideH2"]))
    elif name == "h3":
        story.append(Paragraph(paragraph_markup(el), styles["GuideH3"]))
    elif name == "h4":
        story.append(Paragraph(paragraph_markup(el), styles["GuideH4"]))
    elif name == "p":
        sty = styles["GuideBody"]
        cls = el.get("class") or []
        if "small" in cls:
            sty = styles["GuideSmall"]
        elif "subtitle" in cls:
            sty = styles["GuideSubtitle"]
        elif "callout" in cls:
            sty = styles["GuideCallout"]
        story.append(Paragraph(paragraph_markup(el), sty))
    elif name in {"ul", "ol"}:
        for idx, li in enumerate(el.find_all("li", recursive=False), start=1):
            prefix = "•" if name == "ul" else f"{idx}."
            story.append(Paragraph(f"{prefix} {paragraph_markup(li)}", styles["GuideBullet"]))
        story.append(Spacer(1, 3))
    elif name == "pre":
        raw = el.get_text("", strip=False).strip("\n")
        # Split very long code blocks into chunks to avoid oversized flowables
        lines = raw.split("\n")
        max_lines = 80
        for i in range(0, len(lines), max_lines):
            chunk = "\n".join(lines[i : i + max_lines])
            story.append(Preformatted(chunk, styles["GuideCode"]))
    elif name == "table":
        story.append(build_table(el, styles))
        story.append(Spacer(1, 6))
    elif name == "div":
        cls = el.get("class") or []
        if "pill-row" in cls:
            pills = [span.get_text(" ", strip=True) for span in el.find_all("span", class_="pill")]
            if pills:
                story.append(Paragraph(" | ".join(html.escape(p) for p in pills), styles["GuideSmall"]))
        elif "callout" in cls:
            story.append(Paragraph(paragraph_markup(el), styles["GuideCallout"]))
        elif "interview-q" in cls:
            # Process interview question box children individually
            for child in el.children:
                if isinstance(child, NavigableString) or not isinstance(child, Tag):
                    continue
                process_element(child, styles, story)
        elif "video-links" in cls:
            for nested in el.children:
                if isinstance(nested, NavigableString) or not isinstance(nested, Tag):
                    continue
                process_element(nested, styles, story)
        elif "grid" in cls:
            ...
            text = paragraph_markup(el)
            if text:
                story.append(Paragraph(text, styles["GuideBody"]))
    elif name == "section":
        # Handle wrapped sections (e.g., hero)
        section_items = []
        for child in el.children:
            if isinstance(child, NavigableString) or not isinstance(child, Tag):
                continue
            process_element(child, styles, section_items)
        if section_items:
            if "avoid-break" in (el.get("class") or []):
...
def build_story() -> list:
    ...
    for element in main.children:
        ...
        if not isinstance(element, Tag):
            continue
        process_element(element, styles, story)
...
def main() -> None:
    doc = SimpleDocTemplate(
        ...
        topMargin=18 * mm,
        bottomMargin=16 * mm,
        title="Frontend Interview Concepts — Complete Checklist Guide",
